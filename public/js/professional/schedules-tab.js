...
ScheduleModule = (function ($) {
    ...
    //inputs
    const specialtySelect = $(".schedules_specialty_select");
    ...
    const init = function () {
        ...
        $.when(refreshConfigs()).done(()=>{
            specialtySelect.select2({width: '100%'});
    ...
    const refreshSpecialties = function (data = {}){
        ...
        (uniqueSpecialties.length)
            ?  uniqueSpecialties.filter((spec)=>{
                specialtySelect.append(new Option(spec.name, spec.id))
            })
            : specialtySelect.append(new Option('Selecciona una especialidad', '', 'selected'))
    }
    ...
    const buildFormData = function (){
        ...
        const finalObj = {
            ...
            specialty_id:{
                value: specialtySelect.find(':selected').val(),
                selector:specialtySelect,
                required:true,
            },
            attention_place_id:{
    ...
    const validateForm = function (){
        ...
        if( ! checkValidval(form.specialty_id.value)) {
        ...
        if(isValidForm){
            objectToSend = {
                professional_id: professional.id,
                specialty_id: form.specialty_id.value,
    ...
    const submitForm = function (){
        if(validateForm()){
            callApi(scheduleAddUrl, objectToSend )
        };
    }
